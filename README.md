...
- **다중 키워드 정확도 검색 (AND 연산)**: QueryDSL 기반 동적 쿼리로 여러 검색어 입력 시 모든 키워드를 만족하는 정확도 중심 검색 지원.
...
- **보안 에러 핸들링**: 전역 예외 처리(`@RestControllerAdvice`)를 통한 내부 인프라/스택 트레이스 노출 차단 및 완전한 ORM/Prepared Statement 바인딩.
